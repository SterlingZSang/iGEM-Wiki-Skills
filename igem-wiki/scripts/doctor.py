...
def issue(issues: list[dict[str, object]], code: str, message: str, **details: object) -> None:
...
def validate_local_links(skill_dir: Path, skill_root: Path, issues: list[dict[str, object]]) -> None:
...
def version_at(root: Path | None) -> str | None:
    if root is None:
        return None
    path = root / "VERSION"
    if path.is_file():
...
def build_report(args: argparse.Namespace) -> dict[str, object]:
    requested_root = args.skill_root.resolve()
    skill_root = (
        requested_root.parent
        if requested_root.name in SKILLS and (requested_root / "SKILL.md").is_file()
        else requested_root
    )
    source_root = args.source.resolve() if args.source else None
    present = [skill for skill in SKILLS if (skill_root / skill).is_dir()]
    ...
    return {
        ...
        "installed_version": version_at(skill_root),
...
def render_text(report: dict[str, object]) -> str:
    ...
    if report["source_version"]:
        lines.append(f"Source release: {report['source_version']}")
